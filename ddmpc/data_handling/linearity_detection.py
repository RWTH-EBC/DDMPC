...
from ddmpc.modeling.features.constructed import *
...
def generate_changes(inputs: list[Union[Feature, Source]]) -> list[Source]:
    return [Change(inp) for inp in inputs]


def generate_running_means(inputs: list[Union[Feature, Source]], n_start=2, n_stop=10) -> list[Source]:
...
def generate_running_shifts(inputs: list[Union[Feature, Source]], n_start=2, n_stop=10) -> list[Source]:
...
class LinearityDetector:
    ...
    def _generate_columns(self, inputs: list[Union[Feature, Source]]):
    ...
    def eval(self, inputs: list[Union[Feature, Source]], plot: bool = False) -> pd.Series:

        inputs: list[Source] = _features_to_sources(inputs)

        data = self._generate_columns(inputs)

        res: dict[Source, float] = dict()
