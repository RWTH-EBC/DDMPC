...
from ddmpc.data_handling.reduction import *
from ddmpc.data_handling.storing_data import *
from ddmpc.modeling.features.sources import Constructed, Source
from ddmpc.modeling.predicting import Inputs, Output
...
class TrainingData:
    ...
    def eval_quality(self, labels: list[str] = None, bins: int = None):
        ...
        if labels is None:
            labels = [str(f.source.name) for f in self.inputs]
            labels.append(self.output.source.name)
    ...
    def _process(self, df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
        ...
        def create_lag(data_frame: pd.DataFrame) -> pd.DataFrame:
            ...
            for i in self.inputs:
                ...
                if i.source.col_name not in data_frame.columns:

                    # try to construct the source if it is not included in the data_frame
                    try:
                        data_frame = i.source.process(data_frame)
                    except AttributeError:
                        f'Please make sure {i.source.col_name} is contained in {data_frame.columns}.'

                for k in range(0, i.lag):

                    ret_df[i.col_name(k)] = data_frame[i.source.col_name].shift(k)

            # output
            if not self.output.source.col_name in data_frame.columns:
                if isinstance(self.output.source, Constructed):
                    data_frame = self.output.source.process(data_frame.copy())

            try:
                ret_df[self.output.col_name()] = data_frame[self.output.source.col_name].shift(-1)
            except AttributeError:
                f'Please make sure {self.output.source.col_name} is contained in {data_frame.columns}.'
...
def delete_by_condition(
    ...
    for data_container in to_DataHandler(data):
        ...
        for condition in conditions:
            ...
            if isinstance(left, Source):
                left = df[left.col_name]

            if isinstance(right, Source):
