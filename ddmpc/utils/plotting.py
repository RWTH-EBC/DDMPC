...
class SubPlot:
    ...
    def __init__(
            ...
            enable_right_axis: Optional[bool] = None,
    ):

        self.features: list[Feature] = features

        if unit_format_right or y_label_right or any([f.source.plt_opts.second_axis for f in self.features]):
...
class Plotter:
    ...
    def _build(self, df: pd.DataFrame, current_time: int):
        ...
        for ax_left, sub_plot in zip(axs, self.sub_plots):
            ...
            for feature in sub_plot.features:
                ...
                plt_opt = feature.source.plt_opts

                if feature.source.col_name not in df.columns and isinstance(feature.source, Constructed):
                    try:
                        df = feature.source.process(df)
                    except KeyError:
                        print(f'Could not process {feature}, continuing...')

                try:
                    values = sub_plot.convert(df[feature.source.col_name])

                except KeyError:

                    print(f'Plotting error! Feature with col_name {feature.source.col_name} not in df. Continuing...')
                    continue
    ...
    def plot_solutions(
                ...
                for feature in sub_plot.features:
                    ...
                    if sub_plot.step:
                        ax.step(
                            date_list,
                            values,
                            label=feature.source.plt_opts.label,
                            color=grey,
        ...
        for ax, sub_plot in zip(axs, self.sub_plots):
            ...
            # iterate over all features
            for feature in sub_plot.features:
                if feature.source.col_name not in real.columns:
                    feature.process(real, inplace=True)

                values = sub_plot.convert(real[feature.source.col_name])
